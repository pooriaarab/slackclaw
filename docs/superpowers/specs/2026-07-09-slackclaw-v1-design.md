...
2. **`self`** — Slack session replay via `xoxc` (token) + `xoxd` (cookie), same mechanism already working in the user's solo-admin Slack MCP gateway (`CUSTOM_TLS`, see internal memory `reference-slack-mcp-enterprise-grid`). Hits Slack's internal web-client endpoints as the logged-in user: full DM history, saved items (`stars.list`/saved API), private channels already joined. No bot install or workspace-admin approval needed — only requires extracting an already-valid browser/desktop session.
