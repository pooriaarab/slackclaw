...
**Architecture:** TypeScript/Node CLI (`commander`), `better-sqlite3` for storage with an FTS5 virtual table.

> **Amended 2026-07-09, mid-Task-7:** the original plan below (Tasks 7-8) called for `classic-level` LevelDB parsing plus a Python `ccl_chromium_reader` subprocess to decode Chromium's V8-serialized IndexedDB values. That approach was abandoned before being executed, after discovering `openclaw/slacrawl` (MIT-licensed Go project, same mission, same org as discrawl) had already solved this exact problem more simply: Slack Desktop's message/channel/member state is externalized to **IndexedDB blob files** (Chromium moves large values out of the leveldb proper), and those blobs decode with an optional snappy decompress + **Node's own built-in `v8.deserialize()`** — no LevelDB library, no Python, no native deps needed at all, since our core is already Node (slacrawl has to shell out to `node` for this one step *because* its core is Go; we get it for free in-process). Verified end-to-end against a real local Slack Desktop cache before any code was written: 116 channels with real message history decoded, plus a `saved` key covering saved items that isn't even in slacrawl's desktop-mode coverage. Tasks 6's `locate.ts` was corrected to point at the blob dir; Task 7 is `src/sources/cache/dump.ts` (blob scan + snappy + v8.deserialize, no subprocess); Task 8 is now a redux-state walk (channels/members/messages/saved) rather than a raw-record decode. The actual implementation lives in the two tasks below, updated in place — treat this note as the record of why they don't match what a from-scratch reader would expect from Task 1's original framing.

**Tech Stack:** TypeScript, Node 20+, `commander`, `better-sqlite3`, `snappyjs` (pure JS, zero deps), `vitest` for tests.
